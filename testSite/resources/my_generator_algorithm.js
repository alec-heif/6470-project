...
module.exports.generateNames = function generateNames() {
	...
	fs.readFile('./resources/name_table.json','utf8',function(err,data){
		...
		nameRef.once('value', function(nameSnap) {
			...
			var mixings = ['mix', 'mash', 'combine', 'stir', 'blend', 'broil', 'bake', 'fry', 'steam', 'smoke', 'cook', 'roast', 'put', 'spoon', 'slowly mix', 'inject'];
			...
			for (var i = 0; i < 50000; i++) {
				...
				var ing = addIngredient(recipe, rand(ingredients), ing_names, i, ingredients);
				recipe.name += ing + ' ';
				...
				ing = addIngredient(recipe, rand(ingredients), ing_names, i, ingredients);
				recipe.name += ing;
				if(Math.random() > 0.7)
					recipe.name += ' ' + rand(product);
				var extraIngredients = Math.floor(Math.random()*4);
				for(var j = 0; j < extraIngredients; j++) {
					addIngredient(recipe, rand(ingredients), ing_names, i, ingredients);
				}
				recipe.steps = [];
				for(var l = 0; l < recipe.ingredients.length; l++) {
					var curr = recipe.ingredients[l];
					updateIngredientsLists(recipe, curr, ing_names, i);
				}
...
function addIngredient(recipe, ingredient, ing_names, i, ingredients) {
	if(isInArray(ingredient, recipe.ingredients)) {
		return addIngredient(recipe, rand(ingredients), ing_names, i, ingredients);
	}
	recipe.ingredients.push(ingredient);
	return ingredient;
}

function updateIngredientsLists(recipe, ingredient, ing_names, i) {
	ing_names[ingredient]['recipes'].push(i);
}
